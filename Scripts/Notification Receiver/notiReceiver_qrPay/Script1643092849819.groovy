...
Mobile.takeScreenshotAsCheckpoint('QR Pay Verification')
...
def data = findTestData('MAE Noti').getValue(2, 2)

println(data)

def validate = ((((('You’ve just received RM ' + value) + ' from ') + fullName) + '. REF: ') + data) 

Mobile.verifyElementText(findTestObject('Notification/notiReceiverQRPay_bodyTxt', [('fullName') : fullName, ('value') : value
            , ('RefId') : findTestData('MAE Noti').getValue(2, 2)]), validate)
