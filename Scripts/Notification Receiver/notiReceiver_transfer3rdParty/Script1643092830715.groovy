...
def data = findTestData('MAE Noti').getValue(2, 3)

println(data)

def validate = ((((('You’ve just received RM ' + value) + ' from ') + fullName) + '. REF: ') + data)

Mobile.verifyElementText(findTestObject('Notification/notiReceiverTransfer3rdParty_bodyTxt', [('fullName') : fullName, ('value') : value
            , ('RefId') : findTestData('MAE Noti').getValue(2, 3)]), validate)
